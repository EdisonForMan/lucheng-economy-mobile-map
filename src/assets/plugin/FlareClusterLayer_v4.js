  ...
  var FlareClusterLayer = /** @class */ (function(_super) {
    ...
    function FlareClusterLayer(options) {
      ...
      _this.flareTextSymbol =
        ...
        new TextSymbol({
          ...
          yoffset: -1 // setting yoffset as vertical alignment doesn't work in IE/Edge
        });
      // initial data
    ...
    FlareClusterLayer.prototype._layerViewCreated = function(evt) {
      ...
      if (this.refreshOnStationary) {
        watchUtils.pausable(evt.layerView.view, "stationary", function(
          isStationary,
          b,
          c,
          view
      ...
      if (this._isClustered) {
        for (var i = 0, len = this._gridClusters.length; i < len; i++) {
          if (this._gridClusters[i].clusterCount < this.clusterMinCount) {
            for (
              var j = 0, jlen = this._gridClusters[i].singles.length;
              j < jlen;
              j++
            ) {
              this._createSingle(this._gridClusters[i].singles[j]);
            }
          } else if (this._gridClusters[i].clusterCount > 3) {
    ...
    FlareClusterLayer.prototype._createSingle = function(obj) {
      ...
      //   }
      const { geometry, _symbol,_symbol_point, mode, attributes } = obj;
      ...
      if (this.singleRenderer) {
        ...
        graphic.symbol =
          mode == "point" ? _symbol_point : _symbol;
      }
      this.add(graphic);
    };
        ...
        return __generator(this, function(_b) {
          switch (_b.label) {
            case 0:
              ...
              const count =
                gridCluster.clusterCount < 5
                  ? 24
                  : gridCluster.clusterCount < 10
                  ? 28
                  : gridCluster.clusterCount < 20
                  ? 32
                  : 38;
              const _symbol = {
                type: "picture-marker", // autocasts as new PictureMarkerSymbol()
                url: `${_this.server}/icon/commonIcon/${count}@3x.png`,
                width: count - 2,
                height: count - 2
              };
              ...
              return [
                4 /*yield*/,
                this.clusterRenderer.getClassBreakInfo(cluster.clusterGraphic)
              ];
            case 1:
              // cbi = _b.sent();
              // cluster.clusterGraphic.symbol = cbi.symbol;
              // if (this._is2d && this._activeView.rotation) {
              //   cluster.clusterGraphic.symbol["angle"] =
              //     360 - this._activeView.rotation;
              // } else {
              ...
              return [
                4 /*yield*/,
                this.areaRenderer.getClassBreakInfo(cluster.areaGraphic)
              ];
            case 2:
    ...
    FlareClusterLayer.prototype._createClusterGrid = function(
      webExtent,
      extentIsUnioned
    ) {
    ...
    FlareClusterLayer.prototype._createSurface = function() {
      if (
        ...
        (this._activeView.type === "2d" && !this._activeView.container)
      )
        return;
      var surfaceParentElement = undefined;
      if (this._is2d) {
        surfaceParentElement =
          this._activeView.container.parentElement ||
          this._activeView.container.parentNode;
      } else {
        surfaceParentElement =
    ...
    FlareClusterLayer.prototype._activateCluster = function(cluster) {
      const { clusterGraphic } = cluster;
      const { geometry } = clusterGraphic;
      this.mapView.goTo({
      ...
      // );
      // this.graphics.reorder(
      //   this._activeCluster.textGraphic,
      //   this.graphics.length - 1
      // );
    ...
    FlareClusterLayer.prototype._initSurface = function() {
      ...
      if (!surface) return;
      var sp = this._activeView.toScreen(
    ...
    FlareClusterLayer.prototype._initCluster = function() {
      return __awaiter(this, void 0, void 0, function() {
        ...
        return __generator(this, function(_a) {
          switch (_a.label) {
            case 0:
              if (!this._activeCluster) return [2 /*return*/];
              surface = this._activeView.fclSurface;
              if (!surface) return [2 /*return*/];
              ...
              return [
                4 /*yield*/,
                this._createClonedElementFromGraphic(
                  this._activeCluster.clusterGraphic
                )
              ];
            case 1:
              clonedClusterElement = _a.sent();
              this._activeCluster.clusterGroup.rawNode.appendChild(
                clonedClusterElement
              );
    ...
    FlareClusterLayer.prototype._createClonedElementFromGraphic = function(
      graphic
    ) {
      return __awaiter(this, void 0, void 0, function() {
        ...
        return __generator(this, function(_a) {
          switch (_a.label) {
            case 0:
              return [
                4 /*yield*/,
                symbolUtils.renderPreviewHTML(graphic.symbol)
              ];
            case 1:
              element = _a.sent();
        ...
        return __generator(this, function(_b) {
          switch (_b.label) {
            case 0:
              ...
              subTypeFlares =
                !singleFlares &&
                gridCluster.subTypeCounts &&
                gridCluster.subTypeCounts.length > 0;
              ...
              if (singleFlares) {
                for (i = 0, len = gridCluster.singles.length; i < len; i++) {
                  f = new Flare();
                  f.tooltipText =
                    gridCluster.singles[i][this.singleFlareTooltipProperty];
                ...
                for (i = 0, len = subTypes.length; i < len; i++) {
                  f = new Flare();
                  f.tooltipText =
                    subTypes[i].name + " (" + subTypes[i].count + ")";
                  f.flareText = subTypes[i].count;
                  flares.push(f);
                }
              }
              willContainSummaryFlare = flares.length > this.maxFlareCount;
              flareCount = willContainSummaryFlare
                ? this.maxFlareCount
                : flares.length;
              degreeVariance = flareCount % 2 === 0 ? -180 : -90;
              viewRotation = this._is2d ? this._activeView.rotation : 0;
              clusterScreenPoint = this._activeView.toScreen(
                this._activeCluster.clusterGraphic.geometry
              );
              clusterSymbolSize = this._activeCluster.clusterGraphic.symbol.get(
                "size"
              );
              i_1 = 0;
              _b.label = 1;
            case 1:
              ...
              flareTextAttributes = {};
              isSummaryFlare =
                willContainSummaryFlare && i_1 >= this.maxFlareCount - 1;
              if (isSummaryFlare) {
                ...
                for (
                  j = this.maxFlareCount - 1, jlen = flares.length;
                  j < jlen;
                  j++
                ) {
            ...
            case 2:
              ...
              if (flare.flareText) {
                ...
                textSymbol.text = !isSummaryFlare
                  ? flare.flareText.toString()
                  : "...";
                if (this._is2d && this._activeView.rotation) {
                ...
                return __generator(this, function(_a) {
                  ...
                  switch (_a.label) {
                    case 0:
                      ...
                      this_1._addClassToElement(
                        f.flareGroup.rawNode,
                        "flare-group"
                      );
                      return [
                        4 /*yield*/,
                        this_1._createClonedElementFromGraphic(f.graphic)
                      ];
                    case 1:
                      ...
                      return [
                        4 /*yield*/,
                        this_1._createClonedElementFromGraphic(f.textGraphic)
                      ];
                    case 2:
    ...
    FlareClusterLayer.prototype._setFlarePosition = function(
      flareGroup,
      clusterSymbolSize,
      flareCount,
      flareIndex,
    ...
    FlareClusterLayer.prototype._getFlareSymbol = function(flareGraphic) {
      return __awaiter(this, void 0, void 0, function() {
        return __generator(this, function(_a) {
          switch (_a.label) {
            case 0:
              if (!this.flareRenderer) return [2 /*return*/, this.flareSymbol];
              return [
                4 /*yield*/,
                this.flareRenderer.getClassBreakInfo(flareGraphic)
              ];
            case 1:
              return [2 /*return*/, _a.sent().symbol];
          }
        });
      });
    };
          ...
          switch (_a.label) {
            ...
            case 1:
              ...
              if (flareSymbol.outline) {
                rectShape.setStroke({
                  color: flareSymbol.outline.color,
                  width: 0.5
                });
              }
    ...
    /**
         ...
         * @param callback
         */
    FlareClusterLayer.prototype._addClassToElement = function(
      element,
      className,
      timeoutMs,
      callback
    ...
    FlareClusterLayer.prototype._removeClassFromElement = function(
      element,
      className,
      timeoutMs,
      callback
